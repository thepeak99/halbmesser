...
function getValue(params, name) {
    var match, attrRegex = /^(user|nas|req|res)\|([\w\-]+)$/;
...
function setValue(params, name, newValue) {
    ...
    if (attrRegex.test(name)) {
        match = name.match(attrRegex);
        params[match[1]][match[2]] = newValue;
        return;
    }
...
Chain.prototype.run = function (params) {
    ...
    var idx = 0, elements = this.elements;
    function iterator(r) {
        if (r === false) {
...
Filter.prototype.run = function (params) {
...
Expression.prototype.run = function (params) {
    if (this.op === '&') {
        return this.right.run(params) && this.left.run(params);
    }
